...
import { adminCookie, adminIdentity, allowedRole, equalSecret, requireAdminSession, scopes, signAdminSession } from '@/lib/admin-session'
...
export async function POST(request: NextRequest) {
 ...
 try {
  ...
  return response
 } catch { return NextResponse.json({ error: 'Admin access unavailable. Sign in with your assigned admin account and check server configuration.' }, { status: 403 }) }
}
